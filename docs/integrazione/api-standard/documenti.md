...
Chiamate per creare, aggiornare, cercare e scaricare i documenti delle classi documentali, e per inserirli nei dossier. Per le regole comuni vedi [API standard](index.md).
...
La risposta è direttamente un elenco, con un elemento per ogni documento trovato. Ogni elemento contiene le variabili della classe documentale, i [campi generali](index.md#campi-generali) e l'identificativo del documento. La struttura dipende quindi dalla classe documentale: per questo lo Swagger non la descrive.
